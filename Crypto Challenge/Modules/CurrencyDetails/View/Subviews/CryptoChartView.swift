...
extension CryptoChartView {
    ...
    private func createSubviews() {
        ...
        let (minimum, maximum) = currency.prices.reduce((Float(Int.max), Float(Int.min))) {
        ...
        let firstDate = Date(timeIntervalSince1970: TimeInterval(currency.prices.last?.timestamp ?? 0))
        let lastDate = Date(timeIntervalSince1970: TimeInterval(currency.prices.first?.timestamp ?? 0))
    ...
    private func getChartPoints() -> [ChartPointCandleStick] {
        ...
        for price in currency.prices {
            ...
            let high = price.price + Float.random(in: 0...1000)
            let low = price.price - Float.random(in: 0...1000)
            let open = price.price
            let close = Float.random(in: low...high)
