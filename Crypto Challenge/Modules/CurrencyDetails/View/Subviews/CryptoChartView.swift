...
class CryptoChartView: UIView {
    ...
    @Injected var db: DatabaseService
    
    var prices: [CurrencyPriceEntity] = []
    ...
    init(currency: Currency) {
        ...
        let interval = Int64(Date().timeIntervalSince1970 - 30)
        let predicate = PredicateBuilder(CurrencyPriceEntitySchema.currencyCode.is(currency.code))
            .and(CurrencyPriceEntitySchema.timestamp > interval)
            .build()
        
        prices = db.fetch(CurrencyPriceEntity.self, filter: predicate)
...
extension CryptoChartView {
    ...
    private func createSubviews() {
        ...
        let (minimum, maximum) = prices.reduce((Double.greatestFiniteMagnitude, -Double.greatestFiniteMagnitude)) {
            (min($0.0, $1.price), max($0.1, $1.price))
        }
        let steps = (maximum - minimum) / 10
        let yValues = stride(from: minimum.rounded(.down), through: maximum.rounded(.up), by: steps).map { ChartAxisValueDouble(Double($0), labelSettings: labelSettings) }
        ...
        let firstDate = Date(timeIntervalSince1970: ts - 30)
        let lastDate = Date()
    ...
    private func getChartPoints() -> [ChartPointCandleStick] {
        ...
        for price in prices {
            ...
            let high = price.price
            let low = price.price - (price.price / 10)
            let open = price.price
            let close = price.price
